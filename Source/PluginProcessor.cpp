...
#endif
    ), apvts(*this, nullptr, "Parameters", createParams()) //добавили в констурктор объект класса AudioProcessorValueTreeState
  /*   , parameters(*this, nullptr, "PARAMETERS", intializeGUI())*/

    
#endif
{
    synth.addSound(new SynthSound()); //добавл€ем Sound в синтезеатор
...
//==============================================================================
void TapSynthAudioProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)  //¬ызываетс€ перед началом воспроизведени€, чтобы позволить процессору подготовитьс€.
{
    synth.setCurrentPlaybackSampleRate(sampleRate); //контролирует скорость воспроизведени€
    //visualise.clear();
...
void TapSynthAudioProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
        ...
        buffer.clear(i, 0, buffer.getNumSamples());
    {
        ...
        for (int i = 0; i < synth.getNumVoices(); ++i)//в цикле происходит обновление измен€емых пользователем значений
        {
            if (auto voice = dynamic_cast<SynthVoice*>(synth.getVoice(i))) //проверка на то что voice правильно преобразовалс€
            {
                // Osc
   ...
   //switch ((int)*apvts.getRawParameterValue("MICOMBO_ID"))
   //             {
   //             case 0:
   //                 preXml = juce::parseXML(juce::File("C:/test/3.xml"));
   //        
   ...
   //             case 1:
   //                 preXml = juce::parseXML(juce::File("C:/test/1.xml"));
   //                 apvts.replaceState(juce::ValueTree::fromXml(*preXml));
   //                 break;

   //             case 2:
   //                 preXml = juce::XmlDocument::parse(juce::File("C:/test/2.xml"));
   //                 apvts.replaceState(juce::ValueTree::fromXml(*preXml));
   //                 break;
   //             }
...
//==============================================================================
void TapSynthAudioProcessor::getStateInformation(juce::MemoryBlock& destData) //сохран€€ет и загружает инофрмацию в плагин 
{
...
void TapSynthAudioProcessor::setStateInformation(const void* data, int sizeInBytes) //сохран€ет состо€ние плагина перд закрытием 
{
...
juce::AudioProcessorValueTreeState::ParameterLayout TapSynthAudioProcessor::createParams()
{
    ...
    params.push_back(std::make_unique<juce::AudioParameterChoice>("MICOMBO_ID", "MICOMBO_NAME", juce::StringArray("def", "grap", "loc"), 0));
...
//{
//    std::vector <std::unique_ptr<juce::RangedAudioParameter>> arams;
//    arams.push_back(std::make_unique<juce::AudioParameterChoice>("MICOMBO_ID", "MICOMBO_NAME", juce::StringArray("def", "grap", "loc"), 0));
//    return { arams.begin(), arams.end() };
//}
...
//{
//
//    extern juce::ComboBox presetSelect;
//
//    switch (presetSelect.getSelectedId())
//    {
//    case 0:
//        preXml = juce::XmlDocument::parse(juce::File("C:/test/3.xml"));
//        apvts.replaceState(juce::ValueTree::fromXml(*preXml));
//        break;
//
//    case 1:
//        preXml = juce::XmlDocument::parse(juce::File("C:/test/1.xml"));
//        apvts.replaceState(juce::ValueTree::fromXml(*preXml));
//        break;
//
//    case 2:
